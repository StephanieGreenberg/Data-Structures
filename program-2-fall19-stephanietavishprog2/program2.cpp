...
#ifndef TEST11
{
cerr << "\n\tTEST #11: Test function getval on scanner to get the value of a planet on heap" << endl;
Parser parse("test2.dat");
Planet temp = Planet("Kepler-1049b",0.0);
    vector<Planet> planets = parse.getPlanets();
    Scanner scanner(planets);
    assert(scanner.getval(0) == temp);
    cerr << "\n\t========================PASS========================\n" << endl;
}	
#endif
#ifndef TEST12 
{
    cerr << "\n\tTEST #12: Negative numbers are correctly compared and prioritized in the heap" << endl;
    vector<int> v;
    for(int i = -15; i < 0; i++) v.push_back(i);
    Heap<int> test(v);
    verifyHeap(test);
    cerr << "\n\t========================PASS========================\n" << endl;
}
